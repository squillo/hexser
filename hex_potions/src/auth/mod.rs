...
mod tests {
    ...
    fn signup_happy_path() {
        ...
        let user = execute_signup(&mut repo, SignUpUser { email: "a@b.com".into() }).unwrap();
    ...
    fn signup_rejects_invalid_email() {
        let mut repo = InMemoryUserRepository::default();
        let res = execute_signup(&mut repo, SignUpUser { email: "not-an-email".into() });
        assert!(res.is_err());
    }
    ...
    fn signup_rejects_duplicates() {
        let mut repo = InMemoryUserRepository::default();
        execute_signup(&mut repo, SignUpUser { email: "a@b.com".into() }).unwrap();
        let duplicate = execute_signup(&mut repo, SignUpUser { email: "a@b.com".into() });
