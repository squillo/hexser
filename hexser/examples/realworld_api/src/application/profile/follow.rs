//! Follow user directive and handler.
//!
//! Implements the follow user use case as a Directive (command).
//! Allows a user to follow another user's profile.
//!
//! Revision History
//! - 2025-10-09T23:49:00Z @AI: Initial implementation of follow user.

#[derive(std::clone::Clone, std::fmt::Debug)]
pub struct FollowUserDirective {
    pub follower_id: std::string::String,
    pub followee_username: std::string::String,
}

impl hexser::Directive for FollowUserDirective {
...
pub struct FollowUserHandler<R>
where
...
impl<R> hexser::DirectiveHandler<FollowUserDirective> for FollowUserHandler<R>
where
    R: crate::ports::user_repository::UserRepository,
{
    fn handle(&self, directive: FollowUserDirective) -> hexser::HexResult<()> {
        ...
        if follower.id == followee.id {
            return std::result::Result::Err(
                hexser::Hexserror::validation("Cannot follow yourself")
                    .with_field("followee_username")
                    .with_next_step("Choose a different user to follow")
            );
        }
...
mod tests {
    #[test]
    fn test_follow_user_directive_validation() {
        let directive = super::FollowUserDirective {
    ...
    #[test]
    fn test_follow_user_directive_empty_follower() {
        let directive = super::FollowUserDirective {
            follower_id: std::string::String::from(""),
            followee_username: std::string::String::from("user2"),
        };
