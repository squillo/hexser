...
fn main() -> hexser::HexResult<()> {
  ...
  if let Some(found) = <InMemoryTodoRepository as hexser::ports::repository::QueryRepository<
    Todo,
  >>::find_one(&repo, &TodoFilter::ById(String::from("1")))?
  {
    println!(
      "Found todo: {} (completed: {})",
      found.title, found.completed
    );
  }

  // Complete the todo and save
  if let Some(mut todo) = <InMemoryTodoRepository as hexser::ports::repository::QueryRepository<
    Todo,
  >>::find_one(&repo, &TodoFilter::ById(String::from("1")))?
  {
    todo.completed = true;
