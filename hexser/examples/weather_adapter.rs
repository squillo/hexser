...
impl Forecast {
  fn new(
    ...
    condition: String,
    observed_at_iso: Option<String>,
  ) -> hexser::result::hex_result::HexResult<Self> {
    if city.trim().is_empty() {
      return Result::Err(hexser::error::hex_error::Hexserror::validation_field(
  ...
  fn observed_at_iso(&self) -> Option<&str> {
    self.observed_at_iso.as_deref()
  }
}
...
fn main() -> hexser::result::hex_result::HexResult<()> {
  ...
  let _adapter =
    RestWeatherAdapter::new(std::string::String::from("https://api.example.com/weather"));
