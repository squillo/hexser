...
mod container_tests {
  use hexser::{
    HexResult,
    container::{Container, Provider, Scope},
  };
  ...
  async fn test_async_service_registration_and_resolution() {
    ...
    let service = container
      .resolve_async::<AsyncEmailService>("async_email")
      .await
      .unwrap();
  ...
  async fn test_async_concurrent_resolution() {
    ...
    let handle1 = tokio::spawn(async move {
      container1
        .resolve_async::<ConcurrentService>("concurrent")
        .await
    });
  ...
  async fn test_duplicate_registration_error() {
    ...
    let result = container
      .register("email", provider2, Scope::Singleton)
      .await;
    assert!(result.is_err());
  }
